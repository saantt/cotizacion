...
public class MarcaVehiculoController {
        ...
        @Operation(summary = "Obtener las páginas de todas las marcas de vehículos", description = "Consulta las marcas de vehículos de forma paginada.")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Páginas obtenidas correctamente"),
                        @ApiResponse(responseCode = "500", description = "Error interno del servidor")
        })
        @GetMapping("/page")
        public ResponseEntity<Page<MarcaVehiculoResponseDto>> obtenerTodasMarcaVehiculoPagina(Pageable pageable) {

                return ResponseEntity.ok(
                                marcaVehiculoService.obtenerTodasMarcaVehiculoPagina(pageable));
        }
